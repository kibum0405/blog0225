    ...
    <div>
        ...
        <div v-if="Array.isArray(value)">
            <div v-for="(item, index) in value" :key="index">
                <span>{{ item.postName ? item.postName : '' }}</span>
            </div>
        </div>
        <div v-else>
            <BasePicker v-if="editMode" searchApiPath="posts" searchParameterName="postName"  idField="id" nameField="postName" path="posts" label="PostId" v-model="value" @selected="pick" :editMode="editMode" />
            <div v-else style="height:100%">
                <span>{{ value && value.postName ? value.postName : '' }}</span>
            </div>
        </div>
    </div>
